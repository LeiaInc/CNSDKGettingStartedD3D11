...
#if defined(LEIA_USE_DIRECTX12)
    LEIASDK_API
    void InitializeD3D12(ID3D12Device* device, ID3D12CommandQueue* commandQueue, eLeiaTaskResponsibility _buffersResponsibility, eLeiaTaskResponsibility _shadersResponsibility, eLeiaTaskResponsibility _renderingResponsibility);
    LEIASDK_API
    void GetRenderTargetView(int view, CD3DX12_CPU_DESCRIPTOR_HANDLE* handle);
    LEIASDK_API
    ID3D12Resource* GetRenderTargetResource(int view, D3D12_RESOURCE_STATES* state);
    LEIASDK_API
    void GetDepthStencilView(int view, CD3DX12_CPU_DESCRIPTOR_HANDLE* handle);
    LEIASDK_API
    ID3D12Resource* GetDepthStencilResource(int view, D3D12_RESOURCE_STATES* state);
    LEIASDK_API
    void DoPostProcess(int width, int height, bool blendedInterlace, ID3D12Resource* renderTarget);
    LEIASDK_API
    void DoPostProcessPicture(int width, int height, ID3D12Resource* pictureResource, ID3D12Resource* renderTarget);
    LEIASDK_API
...
private:
    ...
    void DumpGlErrors(const char* scope);
